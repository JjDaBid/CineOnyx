...
const genreList = {
  ...
  fetchDataFromServer(`https://api.themoviedb.org/3/genre/movie/list?api_key=${api_key}${language}`, function ({genres}) {
    ...
    fetchDataFromServer(`https://api.themoviedb.org/3/movie/popular?api_key=${api_key}&page=1${language}`, heroBanner);
  });
...
const heroBanner = function({ results: movieList }) {
  ...
  for(const {title , path} of homePageSections) {
    fetchDataFromServer(`https://api.themoviedb.org/3${path}?api_key=${api_key}&page=1${language}`, createMovieList, title)
  } 
